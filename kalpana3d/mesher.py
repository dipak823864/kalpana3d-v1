...
def compute_mesh_counts(min_bound, max_bound, resolution, sdf_func, iso_level):
    ...
    # Standard Paul Bourke Offsets (Z-up)
...
@njit(parallel=True) # fastmath removed for safety
    ...
    for x in range(res_x):
        for y in range(res_y):
            for z in range(res_z):
                ...
                # Edge connections for Paul Bourke topology (Standard)
                # Edge 0 (v0-v1), Edge 1 (v1-v2), Edge 2 (v2-v3), Edge 3 (v3-v0)
                # Edge 4 (v4-v5), Edge 5 (v5-v6), Edge 6 (v6-v7), Edge 7 (v7-v4)
                # Edge 8 (v0-v4), Edge 9 (v1-v5), Edge 10 (v2-v6), Edge 11 (v3-v7)
